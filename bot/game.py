...
def _render_bingo_card_image(
    ...
    - 5x5 dark rounded-corner cells
    - Numbers in white; marked cells get a green checkmark
    - Red lines drawn through every completed winning line
    ...
    BG_COLOR = (18, 18, 28)
    CELL_DARK = (38, 28, 22)
    CELL_MARKED = (30, 55, 30)
    NUM_COLOR = (230, 230, 230)
    CHECK_COLOR = (80, 210, 80)
    ...
    for idx in range(25):
        x0, y0, x1, y1 = cell_rect(idx)
        color = CELL_MARKED if numbers[idx] in marked else CELL_DARK
        draw.rounded_rectangle([x0, y0, x1, y1], radius=RADIUS, fill=color)

        cx, cy = cell_center(idx)
        num_str = str(numbers[idx])

        if numbers[idx] in marked:
            draw.text((cx, cy - 12), "✓", fill=CHECK_COLOR, font=font_check, anchor="mm")
            draw.text((cx, cy + 14), num_str, fill=NUM_COLOR, font=font_check, anchor="mm")
        else:
            draw.text((cx, cy), num_str, fill=NUM_COLOR, font=font_num, anchor="mm")
...
async def handle_bingo_win(context, room, winner_id, p1, p2, called):
    ...
    if winner_card:
        card_image = _render_bingo_card_image(
            ...
            marked=winner_card["marked_numbers"],
        )
