...
async def cmd_daily(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = await _ensure_user(update)
    reward = await db.claim_daily_reward(user["telegram_id"], update.effective_chat.id)
    if not reward:
...
async def handle_economy_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "daily":
        reward = await db.claim_daily_reward(owner_id, query.message.chat_id)
    ...
    success = (
        await db.deposit_coins(owner_id, amount, query.message.chat_id)
        if action == "deposit"
        else await db.withdraw_coins(owner_id, amount, query.message.chat_id)
    )
    if not success:
...
async def cmd_deposit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not isinstance(amount, int) or amount <= 0:
        ...
        return
    if not await db.deposit_coins(update.effective_user.id, amount, update.effective_chat.id):
        await _reply_economy_photo(
            update,
...
async def cmd_withdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not isinstance(amount, int) or amount <= 0:
        ...
        return
    if not await db.withdraw_coins(update.effective_user.id, amount, update.effective_chat.id):
        await _reply_economy_photo(
            update,
...
async def cmd_bet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if amount > wallet:
        ...
        return
    won = await _settle_bet(update.effective_user.id, amount, update.effective_chat.id)
    if won is None:
...
async def cmd_steal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if received <= 0:
        ...
        return
    success = await db.perform_steal(thief_id, target_id, amount, received, update.effective_chat.id)
