...
The bot includes an owner-controlled tournament system.

### Commands
- `/tournament` — Open the tournament creation wizard (owner only).
- `/announce [tournament_id]` — Announce the latest/specified tournament again to registered users, known groups, and the configured official channel/group (owner only).
- `/tadd <user_id> [tournament_id]` — Manually register a player for a paid tournament (owner only).
- `/tstart [tournament_id]` — Start a tournament immediately with the current registered players, even if the advertised maximum has not been reached (owner only).

### Tournament configuration
Set these Heroku Config Vars:
- `TOURNAMENT_GROUP_ID` — official Bingo group where all tournament matches are played.
- `TOURNAMENT_CHANNEL` — official Telegram channel where join notifications, round winners, and the champion are posted.
- `OWNER_CONTACT_URL` — URL used by the paid tournament **Contact Owner** button.
